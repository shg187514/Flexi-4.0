...
import {
  deletePretestHistory,
  getBatches,
  getPretestHistory,
  importPretest,
  uploadPretestPreview
} from '../services/api'

function PreTestUpload() {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      const response = await getPretestHistory()
  ...
  const handlePreview = async () => {
    ...
    try {
      const response = await uploadPretestPreview(formData)
  ...
  const handleImport = async () => {
    ...
    try {
      const response = await importPretest({
        batch_id: selectedBatchId,
        rows: previewRows,
        file_name: selectedFile?.name || 'pretest_import'
      })
      setMessage(response.data.message || 'Pre-test imported successfully')
      setPreviewRows([])
  ...
  const handleDeleteHistory = async () => {
    ...
    try {
      await deletePretestHistory(confirmDelete.id)
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="mx-auto max-w-7xl space-y-6">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Pre-Test Module</h1>
          <p className="text-sm text-slate-500">Upload pre-test marks from Excel</p>
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.3fr_0.7fr]">
          <section className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            ...
            >
              ...
              />
              <label htmlFor="pretest-upload" className="cursor-pointer">
                <p className="text-sm font-medium text-slate-700">
                  {selectedFile ? selectedFile.name : 'Drag & drop pre-test Excel here or click to browse'}
                </p>
              ...
              <div className="mt-3 max-h-72 space-y-3 overflow-auto">
                {history.map((item) => (
                  <div key={item.id} className="rounded-md border p-3 text-sm">
                    ...
                    <p className="text-slate-500">{item.module_name || 'Pre-Test Upload'} · {item.status}</p>
