...
function DepartmentAllocation() {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      const response = await getDepartmentHistory()
  ...
  const fetchSummary = useCallback(async () => {
    try {
      const response = await getDepartmentSummary(selectedBatchId)
      setSummary(response.data)
    } catch (err) {
      setError('Unable to load department summary')
    }
  }, [selectedBatchId])
  ...
  const handlePreview = async () => {
    ...
    try {
      const response = await uploadDepartmentPreview(formData)
  ...
  const handleImport = async () => {
    ...
    try {
      const response = await importDepartment({
        batch_id: selectedBatchId,
        rows: previewRows,
        file_name: selectedFile?.name || 'department_import'
      })
      setMessage(response.data.message || 'Department allocations imported successfully')
      setPreviewRows([])
  ...
  const handleDeleteHistory = async () => {
    ...
    try {
      await deleteDepartmentHistory(confirmDelete.id)
  ...
  const departmentEntries = Object.entries(summary.department_breakdown || {})

  return (
    <div className="min-h-screen bg-slate-50 p-6">
      <div className="mx-auto max-w-7xl space-y-6">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Department Allocation Module</h1>
          <p className="text-sm text-slate-500">Upload and report trainee department allocations</p>
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.3fr_0.7fr]">
          <section className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
            ...
            >
              ...
              />
              <label htmlFor="department-upload" className="cursor-pointer">
                <p className="text-sm font-medium text-slate-700">
                  {selectedFile ? selectedFile.name : 'Drag & drop department Excel here or click to browse'}
                </p>
                <p className="mt-1 text-xs text-slate-500">Expected columns: Personal No, Department</p>
              </label>
            </div>
            ...
            <div className="mt-4 flex gap-3">
              ...
              <button
                ...
                disabled={!previewRows.length || !selectedBatchId}
              >
                Import Data
              </button>
            </div>
          ...
          <aside className="space-y-6">
            <section className="rounded-xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
              <h2 className="text-sm font-semibold text-slate-700">Department Summary</h2>
              <div className="mt-3 rounded-lg bg-slate-50 p-3">
                <p className="text-slate-500">Total Allocations</p>
                <p className="text-2xl font-semibold">{summary.total_allocations}</p>
              </div>
              <div className="mt-3 space-y-2">
                {departmentEntries.map(([department, count]) => (
                  <div key={department} className="flex items-center justify-between rounded-lg border px-3 py-2 text-sm">
              ...
              <div className="mt-3 max-h-72 space-y-3 overflow-auto">
                {history.map((item) => (
                  <div key={item.id} className="rounded-md border p-3 text-sm">
                    ...
                    <p className="text-slate-500">{item.module_name || 'Department Upload'} · {item.status}</p>
